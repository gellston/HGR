...
hgrapi::v1::memoryPool::memoryPool(std::vector<std::size_t> bins, std::size_t unit_size, double minRequestedToBinRatio) : impl(new impl_memoryPool()) {
	...
	for (auto& bin : bins) {
		...
		auto memory_block = hgrapi::v1::memoryBlock::create(aligned_size);
...
std::size_t hgrapi::v1::memoryPool::computeAlignSize(std::size_t size) {
	auto& unitSize = this->impl->unit_size;
	return (size % unitSize == 0) ? size : (size + (unitSize - (size % unitSize)));
}

hgrapi::v1::memoryToken_ptr hgrapi::v1::memoryPool::findToken(std::size_t requested_size, std::size_t aligned_size) {
	try {
		auto& memory_list = this->impl->inactive[aligned_size];
		if (memory_list.size() == 0) {
			memory_list.push_back(hgrapi::v1::memoryBlock::create(aligned_size));
		}
		...
		auto raw_token = new hgrapi::v1::memoryToken(actual_size, requested_size, buffer, [weak_impl, memory_block, aligned_size] {
		...
		auto token = std::shared_ptr<hgrapi::v1::memoryToken>(raw_token);
		return token;
	}
	catch (...) {
		throw;
...
hgrapi::v1::memoryToken_ptr hgrapi::v1::memoryPool::acquire(const std::size_t& size) {
	try {
...
double hgrapi::v1::memoryPool::minRequestedToBinRatio() {
	return this->impl->minRequestedToBinRatio;
}

void hgrapi::v1::memoryPool::minRequestedToBinRatio(double rate) {
...
hgrapi::v1::memoryPool_ptr hgrapi::v1::memoryPool::create(std::vector<std::size_t> bins, const std::size_t& unit_size, double minRequestedToBinRatio) {
	try {
		auto raw_pointer = new hgrapi::v1::memoryPool(bins, unit_size, minRequestedToBinRatio);
		return std::shared_ptr<hgrapi::v1::memoryPool>(raw_pointer);
	}
	catch (...) {
		throw;
	}
